...
class IPAMAnalyzer:
    ...
    def map_PAMSet(self,file_settings):
        ...
        if('subpop_size' in keys):
            try:
                self.settings['subpopthreshold_size'] = float(file_settings['subpop_size'])
            except:
                print("subpopthreshold badly formatted")
        if('sorting_method' in keys):
            try:
                self.settings['sorting_meth'] = str(file_settings['sorting_method'])
            except:
                print("Unknown sorting method")
        if('sorting_position' in keys):
            try:
                self.settings['sorting_pos'] = int(file_settings['sorting_position'])
            except:
        ...
        if('plots' in keys):
            try:
                self.settings['create_Plots'] = bool(int(file_settings['Plots']))
            except:
                print("Plots badly formatted")
                pass
        if('start_point' in keys):
            try:
                self.settings['start_point'] = int(file_settings['start_point'])
            except:
                print("Startpoint badly formatted")
        if('end_point' in keys):
            try:
                self.settings['orig_end_point'] = int(file_settings['end_point'])
            except:
        ...
        if('errorbars' in keys):
            try:
                self.settings['errorbars'] = bool(int(file_settings['errorbars']))
            except:
                print("Bad format")
        if(self.settings['AOI_mode']=="cell_mask"):
            try:                
                self.settings['cell_mask_fp']=self.job_folder + "/" + file_settings['Cell_mask_fp']
                print(f"Cell_mask_fp: {self.settings['cell_mask_fp']}")
            except:
                print("Cell map fp badly formatted")
        if('global_coordinates' in keys):
            try:
                self.settings['globalcoordinates']=bool(int(file_settings['global_coordinates']))
            except:
        ...
        for fovidx, current_tif in enumerate(tifs):
            ...
            if(len(tif) > 6):
                ...
                if(self.settings['orig_end_point'] == -1):
                    ...
                    tif = tif[4+(self.settings['start_point']*2):4+(self.settings['end_point'])]
                    print(f"Analysing slices: {5+self.settings['start_point']*2}:{self.settings['end_point']}")
                else:
                    self.settings['end_point'] = self.settings['orig_end_point']
                    tif = tif[4+(self.settings['start_point']*2):4+(self.settings['end_point']*2)]
                    print(f"Analysing slices: {5+self.settings['start_point']*2}:{self.settings['end_point']*2}")
            ...
            if(self.settings['border'] > 0):
                yields = [frame[self.settings['border']:imgheight-self.settings['border'],self.settings['border']*2:imgwidth-self.settings['border']*2]for frame in tif]
            ...
            tifffile.imwrite(f'{self.output_folder}/' + f"Yields_{work_name}_{fn}.tif",data = yields_for_img)
            mask = 0
            if("Projection" in self.settings['AOI_mode']):
                ...
                mask = create_Masks_Ft([frame[self.settings['border']:imgheight-self.settings['border'],self.settings['border']*2:imgwidth-self.settings['border']*2]for frame in tif],self.settings['threshold'])
                yields = np.multiply(yields,mask)
            elif("Cell_mask" in self.settings['AOI_mode']):
                print(f"Reading: {self.settings['cell_mask_fp']} as cell mask image")
            ...
            for cnt in cnts:
                ...
                if(mu['m00'] > self.settings['minsize'] and mu['m00'] < self.settings['maxsize']):
                    ...
                    maxY = imgheight-self.settings['border']
                    maxX = imgwidth-self.settings['border']*2
                    ...
                    if(x+self.settings['border']*2 > minX and x+w+self.settings['border']*2 < maxX and y+self.settings['border'] > minY and y+h+self.settings['border'] < maxY):  
                        if(len(size_filt_cnts) > self.settings['cell_limit']):
                            size_filt_cnts.pop(0)
                        size_filt_cnts.append(cnt)
            if(self.settings['cell_dist'] > 1):  
                size_filt_cnts = self.filter_conts(size_filt_cnts)
                
            print(f"Contours remaining filtering with size thresholds: {self.settings['minsize']}-{self.settings['maxsize']} pixels are {len(size_filt_cnts)}")
            ...
            tifffile.imwrite(f'{self.output_folder}/' + work_name + '_' + fn + "cell_mask.tif", cell_mask_out)
            if(DEBUG):
            ...
            for cellidx, cnt in enumerate(size_filt_cnts):
                ...
                if(self.settings['globalcoordinates']):
                    cellcenter = [self.settings['border'] + globalx+int((rect[0]+rect[2])/2)*4,self.settings['border']*2 + globaly+int((rect[1]+rect[3])/2)*4]
                else:
                    cellcenter = [self.settings['border'] + int((rect[0]+rect[2])/2)*4,self.settings['border']*2 + int((rect[1]+rect[3])/2)*4]
                    ...
                    with warnings.catch_warnings():
                        ...
                        try:
                            ...
                            if(cellidx not in zeroedidx):
                                ...
                                print(f"Nan/Zero yield enc. Timeindex: {timeidx+self.settings['start_point']}. Cellindex: {cellidx}. Setting zero")
            ...
            with open(f'{self.output_folder}/' + work_name+'AllYields.csv', mode = 'a', newline="") as tot_file:
                ...
                if(fovidx==0):
                    tot_yield_writer.writerow(list(self.settings.items()) + [str(datetime.date.today())])
    ...
    def plot_values(self,yields, names, jobname, filename, subjob, rows = -1, columns = -1, mode = "Lines", floor = 0.2,legends=True, errorbars=True):
        ...
        try:
            xlim =[0,round((len(yields[0][0])-base)*self.settings['intervall'])]
        except:
        ...
        for idx, circle in enumerate(radii):
            for jdx, compCircle in enumerate(radii):
                ...
                if(cps[jdx] != cps[idx]):
                    ...
                    if (cpdist+radii[idx]+radii[jdx] <= self.settings['cell_dist']):
                        #Discard
    ...
    def subdivide_yield(self, cellyields, floor = 0, threshold_size = 0.25):
        ...
        base=2
        if(self.settings['sorting_meth'] == "Static_Bins"):
            ...
            print(f"Sorting based on percentile. Sorting position: {self.settings['sorting_pos']}. Percentile size: {threshold_size}")
...
parser.add_argument('-batch','--b', dest='batch_flag', action='store_true', help="Enable batch mode")
parser.add_argument('-Debug',action='store_true', help="Switches on verbose output and debug code.")
parser.add_argument('-Interactive','--I',action='store_true', dest = 'Interactive', help="Goes into interactive mode")
...
if(args.Interactive):
    ...
    while(not chosenBatch):
        resp = input("Do you want to analyze more than one IPAM tiff file simultaneously?[Y/N]\n")
        if(resp == "Y"):
            batch = True
            chosenBatch = True
        elif(resp=="N"):
            ...
            print("Aborted.")
    while(not chosenFP):
        FP = input("Please provide a complete filepath to your singular datafile or to the folder where your data is stored.\n")
    ...
    while(not chosenPAMSet):
        resp = input("Is your PAMSet file and your image data in the same folder? [Y/N]\n")
        if(resp =="Y"):
            PAMset = input("Please input the file name for your PAMset file.\n")
            if(batch):
                PAMsetFP = FP+"/"+PAMset
            else:
