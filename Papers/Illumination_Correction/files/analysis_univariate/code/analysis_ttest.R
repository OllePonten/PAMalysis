# Same as analysis.R except that the t-test p-value is reported, instead of 
# Z'-factor. This was requested by a reviewer.
...
f_neg_log10_t_test_p_value <- function(x,y) {
  -log10(t.test(x,y)$p.value)
}

#------ setup
test_type <- 'neg_log10_t_test_p_value'
THRESH_SCORE <- 0
...
for (tag in c("", "_site", "_col", "_row")) { 
  ...
  #------ Compute differences
  v0 <- vecdiff(P0, f_neg_log10_t_test_p_value)
  v1 <- vecdiff(P1, f_neg_log10_t_test_p_value)
...
fnames <- list.files(path = '.',
                     pattern = 'Dp_neg_log10_t_test_p_value', 
                     full.names = T) 
...
p <- p + geom_text(aes(label=sprintf("10^-{%.0f}", Z)), hjust=1.2, parse=T)
p <- p + ylab("-log10(p-value) for Student's t-test") + ylim(0,200)
p <- p + theme_bw() + coord_flip()
p
ggsave("Ce_Tot_neg_log10_t_test_p_value.pdf", p, width=6, height=6)

#------ save Intensity_feature csv
rm(list=ls())
fnames <- list.files(path = '.',
                     pattern = 'Dp_neg_log10_t_test_p_value', 
                     full.names = T) 
D <- do.call("rbind", lapply(fnames, read.csv, header = TRUE)) 
D$test_type <- NULL
names(D) <- c("Feature_id", "Cell_Compartment", "Feature", "neg_log10_t_test_p_value_without_correction", "neg_log10_t_test_p_value_after_correction", "Grouping")
...
write.csv(D, "Intensity_features_neg_log10_t_test_p_value_improvement.csv", row.names=F, quote=F)
